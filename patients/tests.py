...
from patients.models import Patient, Bed, calcDates, PatientMeasurement


class MeasurementTests(TestCase):
    
    fixtures = ['users.json', 'newpatients.json']
    
    def test_patientMeasurements(self):
        pm = PatientMeasurement()
        vals = pm.getAllValuesForPatient(1)
        pprint.pprint(vals)
        self.assertEqual(7, pm.getAllValuesForPatient(1).count())
        
        
    def test_getAllValuesForPhysicianCategory(self):
        pm = PatientMeasurement()
        dic = pm.getAllValuesForPhysicianCategory(8, 2) 
        self.assertEqual(2, len( dic ))
        pprint.pprint(dic)
        dic2 = pm.getAllValuesForPhysicianCategory(9, 2) 
        self.assertEqual(1, len( dic2 ))
        pprint.pprint(dic2)
        
    def test_getMedianValuesForPhysicianCategory(self):
        pm = PatientMeasurement()
        dic = pm.getMedianForPhysicianCategory(8, 2) 
        pprint.pprint(dic)
        
        dic2 = pm.getMedianForPhysicianCategory(9, 2) 
        pprint.pprint(dic2)
       # self.assertEqual(1, len( dic2 ))
